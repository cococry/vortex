...
void vt_util_arena_reset(struct vt_arena_t *a) { a->offset = 0; }
...
char *vt_util_log_get_filepath() {
  ...
  snprintf(logfile, sizeof(logfile), "%s/%s-%s-%d.log", log_dir, _BRAND_NAME,
           timestamp, pid);
...
void vt_util_log_header(FILE *stream, log_level_t lvl) {
  static const char *lvl_str[VT_LL_COUNT] = {"TRACE", "WARNING", "ERROR",
                                             "FATAL"};
  ...
  fprintf(stream,
          ...
          "]: %s%s%s%s: %s%s%s%s: ",
          // log level
          colorize ? lvl_clr[lvl] : "", colorize ? fnt_bold : "", lvl_str[lvl],
          colorize ? clr_reset : "",

          // time (blue, bold)
          colorize ? fnt_bold : "", colorize ? clr_blue : "", timebuf,
          colorize ? clr_reset : "");
}

// handle_noop + vt_util_emit_signal =>
// https://github.com/swaywm/wlroots/blob/master/util/signal.c#L7
...
bool vt_util_allocate_shm_rwro_pair(struct vt_compositor_t *comp, size_t size,
                                    int *rw_fd, int *ro_fd) {
  *rw_fd = -1;
  *ro_fd = -1;
