...
#define VT_ERROR(logstate, ...)                                                     \
  if (!(logstate).quiet) {                                                          \
    do {                                                                            \
      ...
      fprintf(_stream, "%s: %s: ", _SUBSYS_NAME, __func__);                         \
      fprintf(_stream, __VA_ARGS__);                                                \
      fprintf(_stream, "\n");                                                       \
      if ((logstate).stream != stdout &&                                            \
          (logstate).stream != stderr && !(logstate).quiet) {                       \
        vt_util_log_header(stderr, VT_LL_ERR);                                      \
        fprintf(stderr, "%s: %s: ", _SUBSYS_NAME, __func__);                        \
...
#define VT_PARAM_CHECK_FAIL(comp) \
  do { \
    VT_ERROR((comp)->log, "%s: %s: Did not pass parameter check", _SUBSYS_NAME, __func__); \
  } while (0)

#define VT_WL_OUT_OF_MEMORY(comp, client) \
  do { \
    VT_ERROR((comp)->log, "%s: %s: Out of memory.", _SUBSYS_NAME, __func__); \
