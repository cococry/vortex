  ...
  if (!(logstate).quiet) {                                                     \
    if ((logstate).verbose) {                                                  \
      do {                                                                     \
        ...
        if ((logstate).stream != stdout && (logstate).stream != stderr &&      \
            !(logstate).quiet) {                                               \
...
#define VT_WARN(logstate, ...)                                                 \
  if (!(logstate).quiet) {                                                     \
    do {                                                                       \
      ...
      fprintf((logstate).stream, "%s: %s (%s:%d): ", _SUBSYS_NAME, __func__,   \
              __FILE__, __LINE__);                                             \
      fprintf((logstate).stream, __VA_ARGS__);                                 \
      fprintf((logstate).stream, "\n");                                        \
      if ((logstate).stream != stdout && (logstate).stream != stderr &&        \
        ...
        fprintf(stdout, "%s: %s (%s:%d): ", _SUBSYS_NAME, __func__, __FILE__,  \
                __LINE__);                                                     \
        fprintf(stdout, __VA_ARGS__);                                          \
        fprintf(stdout, "\n");                                                 \
      }                                                                        \
...
#define VT_ERROR(logstate, ...)                                                \
  if (!(logstate).quiet) {                                                     \
    do {                                                                       \
      FILE *_stream =                                                          \
          ((logstate).stream == stdout) ? stderr : (logstate).stream;          \
      vt_util_log_header(_stream, VT_LL_ERR);                                  \
      fprintf(_stream, "%s: %s (%s:%d): ", _SUBSYS_NAME, __func__, __FILE__,   \
              __LINE__);                                                       \
      ...
      if ((logstate).stream != stdout && (logstate).stream != stderr &&        \
        ...
        fprintf(stderr, "%s: %s (%s:%d): ", _SUBSYS_NAME, __func__, __FILE__,  \
                __LINE__);                                                     \
