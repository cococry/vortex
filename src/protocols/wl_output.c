...
static void _bind_output(struct wl_client *client, void *data, uint32_t version,
  ...
  VT_TRACE(output->backend->comp->log,
           ...
           "scale=%d "
           "modes=%d",
           (void *)output, (void *)res, (void *)client, id, version, output->x,
           output->y, output->physical.mm_width, output->physical.mm_height,
           ...
           output->physical.transform, output->current_scale,
