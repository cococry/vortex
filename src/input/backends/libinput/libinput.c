...
_vt_li_open_restricted(const char* path, int32_t flags, void* user_data) {
  ...
  if (!session->impl.open_device(session, dev, path)) {
    VT_ERROR(session->comp->log, "Failed to open input device on path '%s'.", path);
    return -errno;
  }
...
_vt_li_log_devices(struct vt_input_backend_li_t *li) {
  ...
  while ((event = libinput_get_event(li->input)) != NULL) {
    ...
    if (type == LIBINPUT_EVENT_DEVICE_ADDED) {
      ...
      char msg[512];
      snprintf(msg, sizeof(msg), "Discovered device: name='%s', sysname='%s', caps: %s %s %s",
               name ? name : "(null)",
...
input_backend_init_li(struct vt_input_backend_t* backend, void* native_handle) {
  ...
  if(backend->comp->backend->platform != VT_BACKEND_DRM_GBM) {
    VT_ERROR(backend->comp->log, "libinput is currently only supported by the DRM sink backend.");
    return false;
  }
  ...
  VT_TRACE(backend->comp->log, "Successfully assigned libinput seat '%s'.", backend->comp->session->name);
  ...
  if(!_vt_li_init_xkb(li)) {
    VT_ERROR(backend->comp->log, "Failed to initialize libinput backend XKB state."); 
    return false;
  }
  
  VT_TRACE(backend->comp->log, "Successfully initialized XKB keyboard backend-global state."); 
  ...
  VT_TRACE(backend->comp->log, "Successfully added the libinput FD to the wayland event loop."); 
...
input_backend_terminate_li(struct vt_input_backend_t* backend) {
  ...
  if(libinput_unref(li->input) != NULL) {
    VT_ERROR(backend->comp->log, "Failed to unref libinput context: %s", strerror(errno));
  }
