...
static void    _vt_session_seat_log_func(enum libseat_log_level level,
...
static struct vt_device_t *
_vt_session_drm_open_device_if_kms(struct vt_session_t *session,
                                   const char          *path);
/*                            The two functions below are equivalent to
 * drmIsKMS() and drmIoctl(), we are self implementing them as to not bring in
 * libdrm as a hard dep.*/
static bool _vt_session_drm_card_is_kms(int32_t fd);
static int  _vt_session_drm_ioctl(int fd, unsigned long request, void *arg);

// Taken from
// https://chromium.googlesource.com/chromiumos/third_party/libdrm/+/df21b293e9cf550ec8d6a3e49461350dbdf14260/xf86drm.c,
// line 174
int _vt_session_drm_ioctl(int fd, unsigned long request, void *arg) {
  int ret;
  do {
...
bool _vt_session_drm_card_is_kms(int32_t fd) {
...
void _vt_session_seat_log_func(enum libseat_log_level level, const char *format,
                               va_list args) {
  if (!_log_comp_ptr)
    return;
  char buf[512];
...
void _vt_session_seat_enable(struct libseat *seat, void *userdata) {
  if (!userdata)
    return;
...
void _vt_session_seat_disable(struct libseat *seat, void *userdata) {
  if (!userdata)
    return;
  struct vt_session_t     *session = (struct vt_session_t *)userdata;
  struct vt_session_drm_t *session_drm =
  ...
  if (libseat_disable_seat(seat) < 0) {
    VT_ERROR(session->comp->log, "Failed to disable seat %s.",
             session_drm->seat_name);
  }
  session->active = false;
  VT_TRACE(session->comp->log, "Seat disabled by libseat");
}

int32_t _vt_session_seat_dispatch(int fd, uint32_t mask, void *data) {
  if (!data)
    return 0;
...
bool _vt_session_drm_card_valid(const char *sysname) {
...
_vt_session_drm_udev_enumerate_cards(struct vt_session_t *session) {
  ...
  // We tell udev that we want to enumerate all primary DRM cards (card0-9), no
  // other devices.
  udev_enumerate_add_match_subsystem(enumerate, "drm");
  udev_enumerate_add_match_sysname(enumerate,
                                   _VT_DRM_PRIMARY_MINOR_NAME "[0-9]*");
...
_vt_session_drm_open_device_if_kms(struct vt_session_t *session,
  ...
  if (!vt_session_open_device_drm(session, dev, path) ||
      !_vt_session_drm_card_is_kms(dev->fd)) {
...
int32_t _vt_session_drm_udev_dispatch(int fd, uint32_t mask, void *data) {
  if (!data)
    return 1;
  struct vt_session_t     *session = data;
  struct vt_session_drm_t *session_drm =
  ...
  struct udev_device *udev_dev =
      udev_monitor_receive_device(session_drm->udev_mon);
  if (!udev_dev)
    return 1;
  ...
  VT_TRACE(session->comp->log, "Received udev event for device %s (%s)", name,
           action);
  ...
  if (session_drm->seat_name[0] != '\0' &&
      strcmp(session_drm->seat_name, seat) != 0) {
    udev_device_unref(udev_dev);
    return 1;
  }
...
bool vt_session_init_drm(struct vt_session_t *session) {
  if (!session || !session->comp)
    return false;

  if (!(session->user_data =
            VT_ALLOC(session->comp, sizeof(struct vt_session_drm_t)))) {
    return false;
  }
  ...
  snprintf(session_drm->seat_name, sizeof(session_drm->seat_name), "%s",
           seat_name);

  // After we opened lé chair, we make sure that lé wayland event loop knows
  // about it
  int32_t fd = libseat_get_fd(session_drm->seat);
  VT_TRACE(session->comp->log, "Initialized seat with libseat fd = %d\n", fd);
  wl_event_loop_add_fd(session->comp->wl.evloop, fd, WL_EVENT_READABLE,
                       _vt_session_seat_dispatch, session);

  if (libseat_dispatch(session_drm->seat, 0) == -1) {
    VT_ERROR(session->comp->log, "Cannot dispatch seat event on seat '%s'.",
             session_drm->seat_name);
    vt_session_terminate_drm(session);
    return false;
  }

  VT_TRACE(session->comp->log, "DRM: Opened Seat '%s'.",
           session_drm->seat_name);
  ...
  session_drm->udev_mon =
      udev_monitor_new_from_netlink(session_drm->udev, "udev");
  ...
  udev_monitor_filter_add_match_subsystem_devtype(session_drm->udev_mon, "drm",
                                                  NULL);
  ...
  wl_event_loop_add_fd(session->comp->wl.evloop, udev_fd, WL_EVENT_READABLE,
                       _vt_session_drm_udev_dispatch, session);
...
bool vt_session_open_device_drm(struct vt_session_t *session,
  ...
  if ((device_id =
           libseat_open_device(session_drm->seat, dev->path, &device_fd)) < 0) {
  ...
  VT_TRACE(session->comp->log, "Opening device %s, device ID: %i", path,
           dev->dev);
...
bool vt_session_manage_device_drm(struct vt_session_t *session,
                                  struct vt_device_t  *dev) {
  if (!dev || !session || !session->user_data)
    return false;
  struct vt_session_drm_t *session_drm =
      BACKEND_DATA(session, struct vt_session_drm_t);
  wl_list_insert(&session_drm->devices, &dev->link);
...
void *vt_session_get_native_handle_drm(struct vt_session_t *session,
  ...
  if (drmGetDeviceFromDevId(dev->dev, 0, &device) != 0) {
    VT_ERROR(session->comp->log,
             "Failed to get native handle of device '%s': "
...
uint32_t vt_session_enumerate_cards_drm(struct vt_session_t *session,
  ...
  if (!(enumerate = _vt_session_drm_udev_enumerate_cards(session)))
    return false;

  // If no cards were enumerated, we wait 10s for primary cards being added to
  // the udev table. (e.g the user happend to plug in his GPU within the next
  // ten seconds after starting the compositor.)
  if (!udev_enumerate_get_list_entry(enumerate)) {
    udev_enumerate_unref(enumerate);
    VT_TRACE(session->comp->log,
             "No DRM devices were found, we now wait for a DRM card device...");
    ...
    // If there were no GPUs found even after the 10s, then why should we care
    // anymore
    if (!(enumerate = _vt_session_drm_udev_enumerate_cards(session)))
      return false;
  }
  ...
  udev_list_entry_foreach(entry, udev_enumerate_get_list_entry(enumerate)) {
    ...
    struct udev_device *udev_dev =
        udev_device_new_from_syspath(session_drm->udev, path);
    if (!udev_dev)
      continue;
    ...
    // (it's basically the GPU the firmware chose as the primary display device
    // when the system started). So we should prioritise it.
    struct udev_device *udev_dev_pci =
    ...
    struct vt_device_t *dev = _vt_session_drm_open_device_if_kms(
        session, udev_device_get_devnode(udev_dev));
    if (!dev) {
      udev_device_unref(udev_dev);
      continue;
    }
